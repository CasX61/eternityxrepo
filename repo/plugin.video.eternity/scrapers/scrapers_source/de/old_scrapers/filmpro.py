...
#2024-06-02
# edit 2024-12-14
# edit 2025-10-18 - Fixed search pattern for new website structure, fixed 'direct' flag
        ...
        if season == 0:
            ...
            try:
                ...
                for title in titles:
                    try:
                        ...
                        if r:
                            ...
                            if len(r) == 0: continue
                            for i in r:
                                # Updated 2025-10-18: New website structure without Qlty class
                                pattern = 'href="([^"]+).*?class="Title">([^<]+).*?class="Year">(\d+)'
                                isMatch, result = cParser.parse(i.content, pattern)
                                if not isMatch: continue
                                sUrl, sTitle, sYear = result[0]
                                sTitle = sTitle.split(' - Der')[0].split(' &#8211;')[0]  # Clean title
                ...
                for link in aResults:
                    ...
                    if isBlocked: continue
                    if sUrl: sources.append({'source': sDomain, 'quality': 'HD', 'language': 'de', 'url': sUrl, 'direct': False, 'prioHoster': prioHoster})
                return sources
            except:
                return sources
