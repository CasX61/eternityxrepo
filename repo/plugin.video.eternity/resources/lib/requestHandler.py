...
# edit 2025-06-12
"""
 RH - edit by kasi - xstream -> xship -> eternity
 2025-04-08
...
# Cloudscraper für Cloudflare Bypass
try:
    import cloudscraper
...
class cRequestHandler:
            ...
            if e.code >= 400:
                ...
                if 'DDOS-GUARD' in str(data):
                    ...
                    if CLOUDSCRAPER_AVAILABLE:
                        logger.info(' -> [requestHandler]: Cloudflare detected, retrying with cloudscraper: ' + self._sUrl)
                        return self.__cloudflare_bypass()
                    else:
                        ...
                        logger.error(' -> [requestHandler]: Failed Cloudflare active (cloudscraper not available): ' + self._sUrl)
    ...
    def __cloudflare_bypass(self):
        """
        Cloudflare Bypass mit cloudscraper
        Wird automatisch aufgerufen wenn Cloudflare erkannt wird
        """
        try:
            ...
            # Apply headers from requestHandler
            scraper.headers.clear()
            for key, value in self._headerEntries.items():
                scraper.headers[key] = value

            # Fix Accept header for JSON APIs (keep HTML default for non-API URLs)
            if '/api/' in self._sUrl or '.json' in self._sUrl:
                scraper.headers['Accept'] = 'application/json, text/plain, */*'

            # Apply timeout
            scraper.timeout = self.requestTimeout

            # Make request (POST if parameters, else GET)
            if self._aParameters:
                if self.jspost:
                    # JSON POST
                    response = scraper.post(
                        self._sUrl,
                        json=self._aParameters,
                    ...
                    response = scraper.post(
                        self._sUrl,
                        data=self._aParameters,
                        timeout=self.requestTimeout
                    )
            else:
                # GET request
                response = scraper.get(self._sUrl, timeout=self.requestTimeout)

            # Set status and real URL
            self._Status = str(response.status_code)
            self._sRealUrl = response.url

            # Get content
            sContent = response.text

            # Apply same formatting as normal request
            if self.__bRemoveNewLines:
                sContent = sContent.replace('\n', '').replace('\r\t', '')
            if self.__bRemoveBreakLines:
                sContent = sContent.replace('&nbsp;', '')

            # Cache result (same as normal request)
            if self.caching and self.cacheTime > 0:
                if self.isMemoryCacheActive:
                    self.__writeVolatileCache(self.getRequestUri(), sContent)
                    self.__writePersistentCache(self.getRequestUri(), sContent)
            ...
            logger.info(' -> [requestHandler]: Cloudflare bypass successful: ' + self._sUrl)
            return sContent

        except Exception as e:
            logger.error(' -> [requestHandler]: Cloudflare bypass failed: %s - %s' % (str(e), self._sUrl))
            if not self.ignoreErrors:
                xbmcgui.Dialog().ok('xShip', 'Cloudflare Bypass fehlgeschlagen: ' + str(e))
            return 'CLOUDFLARE-SCHUTZ AKTIV'
