...
def getHostDict():
    hostblockDict = ['flashx', 'streamlare', 'evoload', 'drop.download', 'supervideo', 'christopheruntilpoint', 'p2pplay', 'secvideo1']  # permanenter Block
...
def getExtIDS(imdb, type): # get external IDS
    ...
    V2_API_KEY = '239a31316d98d8a9b6590a9a8204de30a5056fbeaa5a630b3c5539c06cd37179'
...
def get_titles_for_search(localtitle, title, aliases):
    ...
    try:
        ...
        if localtitle != '':
            ...
            # Add title before " - " (e.g., "Your Name" from "Your Name - Gestern, heute...")
            if ' - ' in localtitle:
                short_title = localtitle.split(' - ')[0].strip()
                if short_title not in titles:
                    titles.append(short_title)
                    titles.append(getsearch(short_title))
        if title != '':
            title = title.lower()
            if localtitle != title:
                titles.append(title)
