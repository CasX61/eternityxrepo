...
class person:
	...
	def personDirectory(self, items):
		...
		for i in items:
			try:
				...
				# Show selection menu: Movies, TV Shows, or Both
				url = '%s?action=personCreditsMenu&sysmeta=%s' % (sysaddon, sysmeta)
	...
	def creditsMenu(self, params):
		"""Menu to choose between movies, TV shows, or both"""
		sysmeta = params.get('sysmeta')
		navigator.navigator().addDirectoryItem("Filme & Serien (Alle)",
			'personCredits&sysmeta=%s&credit_type=combined_credits' % sysmeta,
			'_people-search.png', 'DefaultMovies.png')
		navigator.navigator().addDirectoryItem("Nur Filme",
			'personCredits&sysmeta=%s&credit_type=movie_credits' % sysmeta,
			'movies.png', 'DefaultMovies.png')
		navigator.navigator().addDirectoryItem("Nur TV-Serien",
			'personCredits&sysmeta=%s&credit_type=tv_credits' % sysmeta,
			'tvshows.png', 'DefaultTVShows.png')
		navigator.navigator()._endDirectory('', False)

	def getCredits(self, params):
		try:
			if 'items' in params:
				list = json.loads(params['items'])
				content = params.get('content', 'movies')
				self.creditsDirectory(list, int(params['number']), content)
			else:
				meta = json.loads(params.get('sysmeta'))
				credit_type = params.get('credit_type', 'combined_credits')

				# combined_credits: Beide (Filme + Serien)
				# movie_credits: Nur Filme
				# tv_credits: Nur Serien
				self.list = cTMDB().search_credits(credit_type, meta['id'])
				...
				# Content type für Kodi
				if credit_type == 'movie_credits':
					content = 'movies'
				elif credit_type == 'tv_credits':
					content = 'tvshows'
				else:
					content = 'videos'

				self.creditsDirectory(self.list, 0, content)
				return self.list
		except:
			pass


	def creditsDirectory(self, items, number=0, content='movies'):
		if items == None or len(items) == 0:
			control.idle()
			sys.exit()
		sysaddon = sys.argv[0]
		...
		for i in range(number, number + 20):
			...
			try:
				...
				# Filme direkt abspielbar, Serien zu Staffeln
				mediatype = meta.get('mediatype', 'movie')
				if mediatype == 'movie':
					url = '%s?action=playfromPerson&sysmeta=%s' % (sysaddon, sysmeta)
					isFolder = False
				else:  # tvshow
				...
				try:
					playcount = playcountDB.getPlaycount(mediatype, 'name', label)
		...
		try:
			if i < len(items)-1:
				number = number + 20
				url = '%s?action=personCredits&items=%s&number=%s&content=%s' % (sys.argv[0], control.quote_plus(json.dumps(items)), number, content)
				item = control.item(label="Nächste Seite")
				icon = control.addonNext()
				item.setArt({'icon': icon, 'thumb': icon, 'poster': icon, 'banner': icon})
				if not addonFanart == None: item.setProperty('Fanart_Image', addonFanart)
