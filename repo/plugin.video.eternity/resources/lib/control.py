...
# Aliases for Umbrella compatibility (library integration)
...
addonId = addonInfo('id')	   # 'plugin.video.eternity'
addonName = addonInfo('name')   # 'Eternity'
addonVersion = addonInfo('version')
addonPath = translatePath(addonInfo('path'))   # 'C:\\Program Files\\Kodi21\\portable_data\\addons\\plugin.video.eternity\\'
addonProfilePath = translatePath(addonInfo('profile')) # 'C:\\Program Files\\Kodi21\\portable_data\\userdata\\addon_data\\plugin.video.eternity\\'
...
homeWindow = xbmcgui.Window(10000)  # Alias for Umbrella compatibility
monitor = xbmc.Monitor()  # For library service
currentWindowId = xbmcgui.Window(xbmcgui.getCurrentWindowId())
item = xbmcgui.ListItem
dialog = xbmcgui.Dialog()
progressDialog = xbmcgui.DialogProgress()
progressDialogBG = xbmcgui.DialogProgressBG()
progress_line = '%s[CR]%s'  # For multi-line progress dialogs (like Trakt auth)
...
def refresh():
	...
	Based on Umbrella's refresh() - needed for Trakt integration
	"""
	return execute('Container.Refresh')

def trigger_widget_refresh():
	"""
	Trigger widget refresh after Trakt actions
	Based on Umbrella's trigger_widget_refresh()
	Forces Kodi widgets to update their content
	"""
	# Force library update to trigger widget refresh
	# This is a "fake" path that forces Kodi to refresh all widgets
	execute('UpdateLibrary(video,/fake/path/to/force/refresh/on/home)')

def legalFilename(filename):
	"""
	Convert filename to legal filesystem name
	For library integration - creates safe filenames
	"""
	import re
	try:
...
def existsPath(path):
	...
	For library integration - checks if .strm/.nfo files exist
	"""
	try:
