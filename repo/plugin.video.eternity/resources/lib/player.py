...
class player(xbmc.Player):
    ...
    def run(self, title, url, meta):
        import xbmc
        xbmc.log('PLAYER run: Called with title=%s, url=%s' % (title, url), xbmc.LOGINFO)
        try:
            self.meta = meta
            ...
            self.tvdb = meta.get('tvdb_id', None)  # For TV shows
            ...
            # Handle URLs with headers (|)
            if '|' in url:
                stream_url, strhdr = url.split('|', 1)
                xbmc.log('PLAYER: URL with headers detected. URL: %s, Headers: %s' % (stream_url, strhdr), xbmc.LOGINFO)
                ...
                if ".m3u" in url or '.mpd' in url:
                    ...
                    # For regular MP4/other files with headers, use them directly
                    xbmc.log('PLAYER: Setting path with headers for regular video file', xbmc.LOGINFO)
                    item.setPath(url)
            else:
                if ".m3u" in url or '.mpd' in url:
                    item.setProperty("inputstream", "inputstream.adaptive")
            ...
            xbmc.log('PLAYER run: About to play. sys.argv[1]=%s' % sys.argv[1], xbmc.LOGINFO)
            if int(sys.argv[1]) > 0:
                xbmc.log('PLAYER run: Calling setResolvedUrl', xbmc.LOGINFO)
                xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, item)
            else:
                xbmc.log('PLAYER run: Calling xbmc.Player().play()', xbmc.LOGINFO)
                xbmc.Player().play(url, item)
            xbmc.log('PLAYER run: Calling keepPlaybackAlive()', xbmc.LOGINFO)
            self.keepPlaybackAlive()
            xbmc.log('PLAYER run: keepPlaybackAlive() returned, exiting', xbmc.LOGINFO)
            return
        except Exception as e:
            xbmc.log('PLAYER run: Exception: %s' % str(e), xbmc.LOGERROR)
            import traceback
            xbmc.log('PLAYER run: Traceback: %s' % traceback.format_exc(), xbmc.LOGERROR)
            return
    ...
    def onPlayBackStopped(self):
        ...
        # Trakt scrobble (based on Umbrella's implementation)
        self.traktScrobble()
...
# ====================
    ...
    def traktScrobble(self):
        """
        Trakt Scrobbling - Based on Umbrella's set_scrobble() implementation
        Called when playback stops to save progress or mark as watched
        """
        try:
            # Check if Trakt scrobble is enabled
            ...
            # Get markwatched percentage (default 85%)
            try:
                markwatched_percentage = int(control.getSetting('markwatched.percent')) or 85
            except:
                markwatched_percentage = 85
            ...
            # Check if seekable (> 3 minutes AND < markwatched percentage)
            seekable = (int(self.currentTime) > 180 and (percent < int(markwatched_percentage)))

            # ALWAYS log scrobble info (not just debug mode)
            xbmc.log('[Eternity-Trakt] traktScrobble: mediatype=%s, current_time=%d, total_time=%d, percent=%.2f, seekable=%s, tvdb=%s, season=%s, episode=%s' %
                    (self.mediatype, self.currentTime, self.totalTime, percent, seekable, self.tvdb, self.season, self.episode), xbmc.LOGINFO)

            # Save progress if seekable (< markwatched percentage)
            if seekable:
                scrobbled = False
                if self.mediatype == 'movie' and self.imdb:
                    trakt.scrobbleMovie(self.imdb, '', percent)
                    xbmc.log('[Eternity-Trakt] Scrobbled Movie: %s (%.1f%%)' % (self.title, percent), xbmc.LOGINFO)
                    scrobbled = True

                elif self.mediatype in ('episode', 'tvshow') and self.tvdb and self.season and self.episode:
                    trakt.scrobbleEpisode(self.imdb, '', self.tvdb, self.season, self.episode, percent)
                    xbmc.log('[Eternity-Trakt] Scrobbled Episode: %s S%sE%s (%.1f%%)' %
                            (self.title, self.season, self.episode, percent), xbmc.LOGINFO)
                    scrobbled = True
                else:
                    xbmc.log('[Eternity-Trakt] NOT scrobbling - mediatype=%s, tvdb=%s, season=%s, episode=%s' %
                            (self.mediatype, self.tvdb, self.season, self.episode), xbmc.LOGWARNING)

                # Refresh container after scrobbling to update progress lists
                if scrobbled:
                    xbmc.executebuiltin('Container.Refresh')

            # Mark as watched if >= markwatched percentage
            if percent >= int(markwatched_percentage):
                marked_watched = False
                if self.mediatype == 'movie' and self.imdb:
                    trakt.scrobbleReset(imdb=self.imdb, tmdb='', tvdb=None, season=None, episode=None)
                    xbmc.log('[Eternity-Trakt] Marked as Watched: Movie %s (%.1f%%)' % (self.title, percent), xbmc.LOGINFO)
                    marked_watched = True

                elif self.mediatype in ('episode', 'tvshow') and self.tvdb and self.season and self.episode:
                    trakt.scrobbleReset(imdb=self.imdb, tmdb='', tvdb=self.tvdb, season=self.season, episode=self.episode)
                    xbmc.log('[Eternity-Trakt] Marked as Watched: Episode %s S%sE%s (%.1f%%)' %
                            (self.title, self.season, self.episode, percent), xbmc.LOGINFO)
                    marked_watched = True

                # Refresh container after marking as watched to update progress lists
                if marked_watched:
                    xbmc.executebuiltin('Container.Refresh')

        except Exception as e:
            if self.isdebug:
                xbmc.log('[Eternity-Trakt] Scrobble Error: %s' % str(e), xbmc.LOGERROR)
            pass
