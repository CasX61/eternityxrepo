...
def _checkdomain(_domain, _provider):
    try:
        # Filmpalast & Serienstream vom Check ausschließen wenn DNS Bypass aktiviert
        # (requests umgeht socket.getaddrinfo Patch)
        dns_bypass_enabled = (getSetting('bypassDNSlock', 'false') == 'true')
        if dns_bypass_enabled and _provider in ('filmpalast', 'serienstream'):
            setSetting('provider.%s.check' % _provider, 'true')
            if isLogger: logger.info(' -> [service]: Provider: %s / Check: skipped (DNS Bypass aktiv), Check: true' % _provider)
            return
        ...
        try:
            ...
            if domain in wrongDomain:
                ...
                # Domain NUR speichern wenn DNS Bypass DEAKTIVIERT ist
                # (verhindert IP-Speicherung bei aktiviertem DNS Bypass)
                if not dns_bypass_enabled:
...
def start_library_service():
	"""Start library auto-update service in background thread"""
	try:
		from resources.lib.modules import library
		import threading

		# Start library service in separate thread
		service_thread = threading.Thread(target=library.lib_tools.service)
		service_thread.daemon = True  # Daemon thread will exit when main program exits
		service_thread.start()

		import xbmc
		xbmc.log('[Eternity] Library auto-update service started', xbmc.LOGINFO)
	except:
		import xbmc, traceback
		xbmc.log('[Eternity] Failed to start library service: %s' % traceback.format_exc(), xbmc.LOGERROR)
...
if __name__ == "__main__":
	...
	# Start library auto-update service
